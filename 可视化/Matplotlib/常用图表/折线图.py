...
plt.plot(values[:, 1], values[:, 3], marker="o")  # 散点图数据集1
plt.plot(values[:, 1], values[:, 4], marker="*")  # 散点图数据集2
plt.plot(values[:, 1], values[:, 5], marker="D")  # 散点图数据集3


plt.xticks(values[range(0, 70, 4), 1], rotation=45)  # 设置x轴刻度，字体旋转
...
plt.savefig('Matplotlib-file/2000-2017年各产业生产总值折线图.jpg')
plt.show()
